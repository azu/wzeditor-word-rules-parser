...
function lineParse(line) {
    var lineObject = rawLineParse(line);
    /*
     ...
     */
    // 大文字小文字を補正するパターン
    if (lineObject.beforeField === null && lineObject.afterField) {
        return patternForBeforeFieldOnly(lineObject);
    }
    // 変更前単語 + 変更後単語
    if (lineObject.beforeChar === null && lineObject.afterChar === null) {
...
module.exports.rawLineParse = rawLineParse;
